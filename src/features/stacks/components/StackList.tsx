...
import { Trash2, ArrowRight, Layers, Calendar, Database, Share2 } from 'lucide-react';
import { formatDistanceToNow } from 'date-fns';
import { ExportStackDialog } from './ExportStackDialog';

export function StackList() {
  const { state, actions } = useAppContext();
  const { stacks } = state;
  const [exportStack, setExportStack] = useState<Stack | null>(null);
  ...
  const handleExportClick = (e: React.MouseEvent, stack: Stack) => {
    e.stopPropagation();
    setExportStack(stack);
  };
  ...
  return (
    <>
          ...
          <Card key={stack.id} className="group hover:shadow-md transition-shadow relative overflow-hidden">
             <div className="absolute top-0 right-0 p-4 opacity-0 group-hover:opacity-100 transition-opacity flex gap-2">
               <Button 
                ...
                onClick={(e) => handleExportClick(e, stack)}
                title="Export"
              >
      ...
      <ExportStackDialog 
        open={!!exportStack} 
        onOpenChange={(open) => !open && setExportStack(null)} 
        stack={exportStack} 
      />
    </>
  );
