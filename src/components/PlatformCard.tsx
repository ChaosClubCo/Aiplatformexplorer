    ...
    >
      <div className="flex items-start justify-between p-5 bg-gradient-to-b from-[#FFF8F1] to-transparent">
        <div className="flex items-center gap-3">
          <div className="w-12 h-12 flex items-center justify-center text-[1.75rem] bg-white border border-[#EDE8E3] rounded-xl">
            {platform.logo}
          </div>
        ...
        <div className="mb-4">
          ...
          <div className="flex gap-2 flex-wrap">
            {(['codeGeneration', 'creativeWriting', 'dataAnalysis', 'reasoning', 'agentCapabilities'] as const).map(
              key => (
                <span key={key} className="inline-flex items-center gap-1 px-2 py-1 text-xs bg-[#F5F5F5] rounded-full">
                  <span className="text-[#5C524D]">
                    {key.replace(/([A-Z])/g, ' $1').trim().split(' ')[0]}
                  </span>
                  <span className={`min-w-[16px] text-center ${getScoreClass(platform.scores[key])}`}>
                    {platform.scores[key]}
                  </span>
                </span>
              )
      ...
      <div className="flex gap-2 p-4 border-t border-[#EDE8E3] bg-[#FAFAFA]">
        <button
          ...
          className="flex-1 px-3 py-2 text-xs text-[#231C19] bg-white border border-[#D9D2CC] rounded-lg hover:bg-[#FAFAFA] hover:border-[#B5ADA6] transition-all"
        >
          Details
        </button>
        <a
          ...
          className="flex-1 px-3 py-2 text-xs text-center text-white bg-[#E88A1D] rounded-lg hover:bg-[#D07614] transition-all"
        >
          Visit Site
        </a>
      </div>
